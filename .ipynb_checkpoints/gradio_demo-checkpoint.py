...
def initialize_pipeline():
    ...
    if pipe is None:
        ...
        try:
            ...
            # NOTE: 请修改为你的实际LoRA路径  
            lora_path = "/root/private_data/wangqiqi/MagicOmini/runs/totbs8_1GPU_512_512_sketch_Prodigy/ckpt/30000"
...
def create_masked_image_from_sketch(base_image, sketch_data):
    ...
    try:
        ...
        base_image = base_image.resize((512, 512)).convert('RGB')
        ...
        # 调整尺寸和格式
        edited_image = edited_image.resize((512, 512)).convert('RGB')
...
def generate_image(base_image, sketch_data, prompt, num_steps, guidance_scale):
    ...
    try:
        ...
        base_resized = base_image.resize((512, 512)) if base_image else Image.new("RGB", (512, 512), (255, 255, 255))
        concat_image.paste(base_resized, (0, 0))
        concat_image.paste(masked_image, (512, 0))
...
def update_sketch_pad(base_image):
    """当上传新图像时，更新绘制画布的背景"""
    if base_image is None:
        return np.ones((512, 512, 3), dtype=np.uint8) * 255  # 白色背景
    
    # 将PIL图像转换为numpy数组
    if isinstance(base_image, Image.Image):
        # 调整到512x512并转换为numpy数组
        resized_image = base_image.resize((512, 512)).convert('RGB')
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                sketch_pad = gr.ImageEditor(
                    ...
                    value=np.ones((512, 512, 3), dtype=np.uint8) * 255  # 初始白色背景
                )
